...
> `suggest` ต้อง deploy ด้วย `--no-verify-jwt` เพราะ token ที่ frontend ส่งมา
> เป็น JWT ที่เรา mint เอง (เซ็นด้วย `SB_JWT_SECRET`) ไม่ใช่ token จาก Supabase Auth —
> ฟังก์ชันตรวจลายเซ็นเองข้างใน (`verifySupabaseJwt`) ก่อนทำงานทุกครั้ง
